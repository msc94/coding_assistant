...
async def test_no_truncate_blocks_large_output_by_default():
    ...
    await handle_tool_call(tool_call, agent, NullCallbacks(), no_truncate_tools=set(), ui=DummyUI())
...
async def test_no_truncate_allows_large_output_for_matching_tools():
    ...
    await handle_tool_call(tool_call, agent, NullCallbacks(), no_truncate_tools={r"^fake_tool\.big_"}, ui=DummyUI())
