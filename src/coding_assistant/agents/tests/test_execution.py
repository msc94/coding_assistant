...
async def test_multiple_tool_calls_are_parallel():
    ...
    await handle_tool_call(msg.tool_calls[1], agent, NullCallbacks(), no_truncate_tools=set(), ui=make_ui_mock())
    ...
    events.clear()
    start = time.monotonic()
    await handle_tool_calls(msg, agent, NullCallbacks(), no_truncate_tools=set(), ui=make_ui_mock())
