...
async def test_unknown_result_type_raises():
    ...
    with pytest.raises(KeyError, match=r"WeirdResult"):
        await handle_tool_call(tool_call, ctx, NullProgressCallbacks(), tool_callbacks=NullToolCallbacks(), ui=make_ui_mock())
...
async def test_multiple_tool_calls_are_parallel():
    ...
    await handle_tool_call(msg.tool_calls[0], ctx, NullProgressCallbacks(), tool_callbacks=NullToolCallbacks(), ui=make_ui_mock())
    await handle_tool_call(msg.tool_calls[1], ctx, NullProgressCallbacks(), tool_callbacks=NullToolCallbacks(), ui=make_ui_mock())
